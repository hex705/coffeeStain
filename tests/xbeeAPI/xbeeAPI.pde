...
XBeeAddress64 desk = new XBeeAddress64(0x00,0x13,0xa2,0x00,0x40,0x70,0x7e,0xee);
...
void keyPressed() {
  ...
  try {
    ...
    //request = new RemoteAtRequest(desk, "DN", ByteUtils.stringToIntArray("USB"));
    ...
    //request = new RemoteAtRequest(desk, "IR", ByteUtils.convertInttoMultiByte(30*1000));
    //xbee.sendAsynchronous(request);
    ...
    request = new RemoteAtRequest(desk, "IS");
    xbee.sendAsynchronous(request);
  }
  catch (Exception e) {
    e.printStackTrace();  
